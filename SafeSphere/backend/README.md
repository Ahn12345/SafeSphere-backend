...
app/
  ...
  routers/
    ...
    external.py       # 공공데이터포털 프록시 (재난문자/지진대피소/화학사고대피소)
  services/
    external_apis.py  # 공공데이터포털 API 호출 래퍼 (httpx)
```
...
- 공공데이터포털 프록시 3종 (`GET /external/disaster-texts`, `/external/earthquake-shelters`, `/external/chemical-shelters`)
  - **서비스키가 없어서 지금은 호출하면 502 에러가 정상**입니다. 키 발급 전까지는 구조 확인용.

## 공공데이터 연동 관련 TODO

1. data.go.kr에서 아래 3개 API 활용신청 → 승인 후 서비스키 발급
   - 행정안전부_긴급재난문자 (15134001)
   - 지진 옥외대피장소 (15138870)
   - 화학사고 대피장소 (15139645) — SafeSphere 핵심 시나리오용, 우선순위 가장 높음
2. 승인 후 각 API 상세페이지의 Swagger 문서에서 **정확한 요청주소·파라미터명**을 확인하고
   `app/services/external_apis.py`의 `TODO` 주석 부분(BASE_URL, params) 교체
3. `.env`에 발급받은 서비스키를 `DISASTER_TEXT_API_KEY` 등에 채우기
4. 소방청 화재정보서비스(15077644)는 실시간성이 없는 통계 API라 이번 스코프에서는 제외함
